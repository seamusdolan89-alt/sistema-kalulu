...
- **Stock (18/9/2026):** `stock.cantidad` es una CACHE de la suma de `stock_movimientos.delta`. **Nunca escribas la tabla `stock` directo** (`UPDATE/INSERT stock`): usá `SGA_DB.moverStock({productoId, sucursalId, delta, tipo, refTipo, refId, motivo})`, `SGA_DB.setStockAbsoluto(...)` para "poner el stock en N", o `aplicarStockSync` (solo el receptor de `sync.js`). Cada cambio deja un movimiento inmutable y marca la fila para sincronizar; una corrección es un movimiento nuevo de signo contrario, nunca un UPDATE ni un DELETE del movimiento. `crearSiNoExiste:false` conserva lo que hacían venta/anulación/devolución/ajustes (sin fila de stock no se toca nada). `SGA_DB.verificarIntegridadStock()` lista las filas donde la cache no cuadra con los movimientos (tiene que dar `[]`); `tests/e2e/test_stock_ledger.py` falla si algún archivo fuera de `db.js` escribe `stock`. Etapa 1 = local a cada compu; la etapa 2 sincroniza los movimientos en vez del valor absoluto.
