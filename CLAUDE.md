...
- **`git cherry` sobrecuenta:** un commit promovido con conflictos resueltos a mano tiene otro patch-id y sigue apareciendo como `+` (faltante) aunque ya esté en `main`. Antes de listar "pendientes de producción": `git fetch`, cruzá por asunto (`git log origin/main --format=%s`) y probá cada uno con `git cherry-pick --no-commit` en un worktree temporal de `origin/main` (vacío = ya está). Los commits dependen unos de otros (ej. uno que edita un bloque que agregó otro): probá también la **cadena en orden cronológico** en el worktree para ver dónde se corta. Los bumps de `?v=N` se reemplazan por UNO al final. Ojo con un `Revert` cuyo `Reapply` ya está en `main`: promoverlo solo deshace la función.
