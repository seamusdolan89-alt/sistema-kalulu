const Productos = (() => {
  ...
  const setupColumnMapping = () => {
    ...
    const autoMappings = {
      ...
      'producto_madre_codigo': ['producto madre codigo', 'madre codigo', 'cod madre'],
      'proveedor_alternativo': ['proveedor alternativo', 'prov alternativo'],
      'pedido_sugerido_cantidad': ['pedido sugerido cantidad', 'pedido cantidad'],
      'pedido_sugerido_unidad': ['pedido sugerido unidad', 'pedido unidad']
    };
  ...
  const importarDesdeExcel = (filas, sucursal_id) => {
    ...
    for (const fila of filas) {
      ...
      try {
        ...
        const unidad_compra  = String(fila.unidad_compra  || 'Unidad').trim();
        const uxpc           = fila.unidades_por_paquete_compra !== '' && fila.unidades_por_paquete_compra != null
                                 ? (parseFloat(fila.unidades_por_paquete_compra) || 1) : 1;
        const unidad_venta   = String(fila.unidad_venta   || 'Unidad').trim();
        const precio_lista_por = String(fila.precio_lista_por || 'Por unidad de venta').trim();
        const precio_lista_divisor = fila.precio_lista_divisor !== '' && fila.precio_lista_divisor != null
                                 ? (parseFloat(fila.precio_lista_divisor) || 1) : 1;

        // Madre product via barcode
        const madreCodStr = String(fila.producto_madre_codigo || '').trim();
        let nuevo_madre_id;  // undefined = don't change; null = clear
        if (madreCodStr) {
          const mRow = window.SGA_DB.query('SELECT producto_id FROM codigos_barras WHERE codigo = ?', [madreCodStr]);
          nuevo_madre_id = mRow.length ? mRow[0].producto_id : null;
        }

        // Alt provider
        const altProvNombre = String(fila.proveedor_alternativo || '').trim();
        const proveedor_alternativo_id = altProvNombre ? getOrCreateProveedor(altProvNombre) : null;

        // Pedido sugerido
        const cantPedido   = fila.pedido_sugerido_cantidad !== undefined && fila.pedido_sugerido_cantidad !== ''
                               ? (parseFloat(fila.pedido_sugerido_cantidad) || 0) : null;
        const pedidoUnidad = String(fila.pedido_sugerido_unidad || '').trim() || null;
        ...
        if (producto_id) {
          // UPDATE existing — build SET clause; producto_madre_id only updated if mapped
          ...
          const setVals = [
            nombre, categoria_id, proveedor_id, costo, precio_venta, stock_minimo,
            unidad_compra, uxpc, unidad_venta, precio_lista_por, precio_lista_divisor,
            proveedor_alternativo_id,
          ];
          if (cantPedido !== null)       { setParts.push('cant_pedido = ?');    setVals.push(cantPedido); }
          if (pedidoUnidad !== null)     { setParts.push('pedido_unidad = ?');  setVals.push(pedidoUnidad); }
          ...
          window.SGA_DB.run(`
            INSERT INTO productos (
              id, nombre, categoria_id, proveedor_principal_id, proveedor_alternativo_id,
              costo, precio_venta, stock_minimo,
              unidad_compra, unidades_por_paquete_compra, unidad_venta,
              precio_lista_por, precio_lista_divisor,
              cant_pedido, pedido_unidad, producto_madre_id,
          ...
          `, [
            producto_id, nombre, categoria_id, proveedor_id, proveedor_alternativo_id,
            costo, precio_venta, stock_minimo,
            unidad_compra, uxpc, unidad_venta, precio_lista_por, precio_lista_divisor,
            cantPedido, pedidoUnidad, nuevo_madre_id !== undefined ? nuevo_madre_id : null,
            now, now, now
          ]);
          results.importados++;
        }
